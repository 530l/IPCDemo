            ...
            BluetoothAdapter.ACTION_STATE_CHANGED -> { // 监听蓝牙状态
                when (val state = intent.getIntExtra(BluetoothAdapter.EXTRA_STATE, 0)) {
                    BluetoothAdapter.STATE_TURNING_ON -> {
                        Log.e("HLQ", "----> STATE_TURNING_ON 蓝牙开启中")
                    }
                    BluetoothAdapter.STATE_ON -> {
                        Log.e("HLQ", "----> STATE_ON 蓝牙开启")
                    }
                    BluetoothAdapter.STATE_CONNECTING -> {
                        Log.e("HLQ", "----> STATE_CONNECTING 蓝牙连接中")
                    }
                    BluetoothAdapter.STATE_CONNECTED -> {
                        Log.e("HLQ", "----> STATE_CONNECTED 蓝牙已连接")
                    }
                    BluetoothAdapter.STATE_DISCONNECTING -> {
                        Log.e("HLQ", "----> STATE_DISCONNECTING 蓝牙断开中")
                    }
                    BluetoothAdapter.STATE_DISCONNECTED -> {
                        Log.e("HLQ", "---->STATE_DISCONNECTED 蓝牙已断开")

                    }
                    BluetoothAdapter.STATE_TURNING_OFF -> {
                        Log.e("HLQ", "---->STATE_TURNING_OFF 蓝牙关闭中")
                    }
                    BluetoothAdapter.STATE_OFF -> {
                        Log.e("HLQ", "---->STATE_OFF 蓝牙关闭")
                    }
